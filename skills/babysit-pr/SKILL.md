...
Keep an eye on changes to main and rebase when needed. If an overlapping PR makes this one obsolete, stop monitoring, report to the user, and ask before closing unless closure was explicitly authorized. If a review bot leaves feedback you believe is not worth addressing, reply and resolve the comment.

Format comments left on Diar's behalf as:

```
[MODEL-SLUG] RESPONDING ON BEHALF OF DIAR
-----

[actual reply]
```

Screenshots and videos help as well. Attach them with whatever the harness already supports; there is no `file-upload` skill.
